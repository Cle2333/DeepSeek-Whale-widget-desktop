...
function createWhaleCore({ dataFile }) {
  ...
  // 回退路径：主路径（EXE 同目录）不可写时用
  const fallbackFile = path.join(os.homedir(), '.dsh-whale-widget-userdata.json')

  function readData() {
    // 两个路径都可能存在，取 mtime **较新**者。
    // ★ 不能「主路径优先」：主路径存在但不可写时（安装目录变只读、文件被置只读属性、
    //   随安装包分发了初始 userdata.json），新设置只会写进回退文件，
    //   而读取永远被陈旧的主文件遮蔽 —— 用户设置被静默丢弃，且每次启动都重复
    //   「清理 → 落盘」却始终不生效
    let best = null
    let bestTime = -1
    for (const p of [file, fallbackFile]) {
      try {
        const st = fs.statSync(p)
        const parsed = JSON.parse(fs.readFileSync(p, 'utf8'))
        if (parsed && typeof parsed === 'object' && st.mtimeMs > bestTime) {
          best = parsed
          bestTime = st.mtimeMs
        }
      } catch (err) {}
  ...
  function save() {
    ...
    // 写失败（例如 EXE 目录只读）时回退到用户目录；
    // readData() 也会读这个回退文件，所以是真双路径而非只写不读
    try {
  ...
  if (purged) {
    // 一并归一版本号：Object.assign(defaultData(), 磁盘数据) 会让旧 version 覆盖默认值，
    // 落盘后仍是旧版本号，后续若按 version 分支迁移会判断失效
