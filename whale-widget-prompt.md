...
> 来源：本仓库由 GitHub 仓库 [MeteorNOX/DeepSeek-Balance-Whale-Widget](https://github.com/MeteorNOX/DeepSeek-Balance-Whale-Widget) 的 DSH 插件改造而来（MIT License，Copyright (c) 2026 MeteorNOX / EthanMaven），本仓库是其中的**桌面版**（Electron，脱离 DSH）。
...
> **桌面版**：本仓库即**脱离 DSH 的 Electron 桌面应用**（打包为 EXE、数据存 EXE 同目录 `userdata.json`），规格差异见文末「附录：桌面版」。

---

## 〇、桌面版速览（脱离 DSH）

本仓库是把上方 DSH 插件**改造成独立桌面应用**的版本（Electron 31，打包 portable EXE）。核心差异：

| 维度 | DSH 插件版（lib/index.js） | 桌面版（本仓库） |
|---|---|---|
| 运行载体 | DSH Web 页面（宿主注入） | 独立透明置顶窗口（560×840，无边框） |
| 余额/配置接口 | `ctx.webServer.register` 路由 `/dsh-whale/*` | 主进程 `ipcMain` + `window.whaleAPI`（preload contextBridge） |
| API_KEY 来源 | DSH 凭据服务 `DEEPSEEK_API_KEY` | **汉堡菜单内填写**（AES-256-GCM 加密后写入 `userdata.json`） |
| 平台令牌 | 凭据服务 `DEEPSEEK_PLATFORM_TOKEN` | 汉堡菜单「平台令牌」行（可选，加密存储） |
| 数据文件 | `$DSH_HOME/.dshw-size.json` / `.dshw-usage.json` | **EXE 同目录 `userdata.json`**（settings/pos 明文，secrets 加密） |
| 每轮对话消耗 | 监听 `session/event` 真实 usage | **无数据源，功能移除**（桌面版菜单不再显示该行） |
| 拖拽/吸附 | 页面内 left/top + CSS 过渡 | 主进程移动窗口 + 四分之一屏边缘吸附（`whale:dragEnd`） |
| 鼠标穿透 | 页面内 pointer-events | `setIgnoreMouseEvents(true, {forward:true})` 透明区穿透 |

架构映射：
- `lib/index.js` 的宿主逻辑（余额拉取、账本、峰谷定价、token 用量换算）→ `lib/core.js`（纯 Node，可单测）。
- `lib/index.js` 的 `WIDGET_JS` 模板字符串 → `renderer/widget.js`（原 `desktop/scripts/extract-widget.mjs` 提取后做桌面化改造，`fetch('/dsh-whale/*')` 全部换成 `API.*` IPC 调用；本仓库直接维护该文件）。
- 音效/图片：`renderer/assets/` 本地文件，`new Audio('./assets/Ya1.mp3')` 之类相对路径。
- 构建：`npm install && npm run dist` → `dist/DeepSeekWhaleWidget_<version>_windows_x86.exe`。
  - 国内网络需先 `set ELECTRON_BUILDER_BINARIES_MIRROR=https://npmmirror.com/mirrors/electron-builder-binaries/`。
  - `npm run dist` 会先跑 `scripts/prepare-electron.mjs`（rcedit 给 `build/electron-dist/electron.exe` 打图标/版本补丁，`electronDist` 指向该目录），再 `electron-builder --win portable`；整套流程无需管理员权限（绕开 winCodeSign 符号链接权限问题）。
  - 冒烟测试：`npx electron . --smoke`（5 秒后自动退出并打印渲染层状态）；打包产物可用 `dist\DeepSeekWhaleWidget_<version>_windows_x86.exe --smoke` 端到端验证。
- **拖拽防抖/防放大**：Windows 透明（分层）窗口在 `setPosition` 时存在尺寸漂移（每次移动"长大"几像素），且窗口移动会触发合成 pointermove 造成位移回弹振荡。因此：移动一律用 `setBounds` 显式钉住宽高（`moveWindowTo`）+ 16ms 帧合并节流；渲染层用 `movementX/Y` 累加真实位移（合成事件为 0，天然过滤）+ `setPointerCapture` 防快速拖动丢事件；挂件尺寸 `--dshw-base` 只随 scale 变化、不依赖窗口视口。

安全说明：`userdata.json` 中 `secrets.apiKey / secrets.platformToken` 用 AES-256-GCM 加密，密钥由「内置 pepper + 本机标识（hostname|user|MAC）」经 PBKDF2-SHA256 派生。属于防明文直读的混淆级保护（非军规级）；渲染层永远拿不到明文密钥，余额请求只发生在主进程。
